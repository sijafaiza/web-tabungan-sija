...
function cetakMutasi() {
    ...
    const transferKeluar = transferData.filter(t => t.dariRek === norek).map(t => ({
        tanggal: t.tanggal,
    ...
    const transferMasuk = transferData.filter(t => t.keRek === norek).map(t => ({
        tanggal: t.tanggal,
    ...
    html += `
            ...
            <tfoot>
                <tr style="font-weight: bold; background: #f0f0f0;">
                    ...
                    <td style="text-align: right; padding: 15px;">${formatRupiah(nasabah.saldo)}</td>
                </tr>
            </tfoot>
        </table>
    ...
    addLog(`Mencetak mutasi rekening ${nasabah.nama} (${norek})`);
}
